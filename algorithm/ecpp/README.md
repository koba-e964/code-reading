...
与えられた正の整数 N (おそらく素数) の素数性を証明したい。

以下の定理が重要。

### [[AO1993]] Thm. 5.2 + Cor. 5.1: 
N を 6 と互いに素な正の整数、E を $\mathbb{Z}/N\mathbb{Z}$ 上の楕円曲線、 $P \in E$ をその上の点、m と s を $s | m$ を満たす正の整数とする。q | s なる素因数 q それぞれに対し $(m / q)P = (x_q : y_q : z_q)$ となるように $x_q, y_q, z_q$ を定める。さらに $mP = O_E$ かつすべての q に対し $\gcd(z_q, N) = 1$ を仮定する。
...
3. $\mathbb{Q}(\sqrt{-D})$ の単数は $w = w(-D)$ 個あるが、それらを $\epsilon _ 1, \ldots, \epsilon _ w$ とする。 $m_i = N(\epsilon _ i \pi - 1)$ としうまく素因数分解できる $m_i$ を見つける。見つからなければ 2. に戻る。ここでの条件は $m_i = FN'$ であって、 $F$ は素因数分解が完全にできている整数、N' はおそらく素数であり $N' \gt (N^{1/4} + 1)^2$ をみたす整数であるようなものである。
...
## 楕円曲線の構成
この節では ECPP で重要な楕円曲線の構成を説明する。実際にここで構成した楕円曲線で計算する必要がある。
...
4
```
そのため $k = J/(J-1) \equiv 7 \pmod{17}$ が成立する。c = 3 とするとこれは mod 17 で平方非剰余であるため、 $y^2 = x^3 - 3kc^{2r} + 2kc^{3r}$ は以下のようになる:
